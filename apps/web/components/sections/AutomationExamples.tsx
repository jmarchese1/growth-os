...
const rowBg = ['bg-transparent', 'bg-white/[0.04]'];
      ...
      <div className="bg-grid-dark pt-20 pb-10 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <h2 className="text-4xl md:text-6xl font-bold tracking-tight leading-tight mb-4">
            When this happens,{' '}
            <span className="text-indigo-400">that</span> happens.
          </h2>
      ...
      <div className="px-6 pb-12">
        <div className="max-w-7xl mx-auto">
          {automations.map((a, i) => (
            <div
              key={i}
              className={`py-6 grid grid-cols-1 md:grid-cols-2 gap-8 -mx-6 px-6 border-b border-white/[0.06] last:border-0 group transition-all duration-200 ${rowBg[i % 2]}`}
            >
              <div>
                <p className="text-xs font-semibold tracking-wider uppercase text-gray-500 mb-2">
                  Trigger
                </p>
                <p className="text-lg font-semibold text-white">{a.trigger}</p>
              </div>
              <div>
