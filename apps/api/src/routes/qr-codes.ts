...
export async function qrCodeRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { token: string }; Querystring: { fp?: string } }>('/qr-codes/public/:token', async (request, reply) => {
    ...
    if (qrCode.cooldownPeriod && isCooldownPurpose) {
      ...
      if (window !== undefined) {
        // Match by fingerprint AND IP together — same device on same network.
        // AND prevents shared WiFi from blocking other people (different fingerprint).
        // Fallback to IP-only when no fingerprint is provided.
