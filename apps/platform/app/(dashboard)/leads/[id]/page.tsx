...
const statusConfig: Record<string, { label: string; bg: string; text: string; border: string }> = {
  ...
  CONVERTED:      { label: 'Converted',      bg: 'bg-violet-500/15',  text: 'text-violet-400',  border: 'border-violet-500/25' },
};
...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const cfg = statusConfig[prospect.status] ?? { label: prospect.status, bg: 'bg-slate-500/10', text: 'text-slate-400', border: 'border-slate-500/20' };
    ...
    <div className="p-8 space-y-8 animate-fade-up">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <Link href="/leads" className="text-slate-600 hover:text-white transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
          <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-4">Contact Information</h3>
          <div className="space-y-3">
            {prospect.email && (
              <div className="flex items-center gap-3">
                ...
                <a href={`mailto:${prospect.email}`} className="text-sm text-violet-400 hover:text-violet-300">{prospect.email}</a>
                {prospect.emailSource && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-slate-500 border border-white/10">
            ...
            {prospect.phone && (
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-300">{prospect.phone}</span>
              </div>
            )}
            {prospect.website && (
              <div className="flex items-center gap-3">
                ...
                <a href={prospect.website} target="_blank" rel="noopener noreferrer" className="text-sm text-violet-400 hover:text-violet-300">{prospect.website}</a>
              </div>
            )}
            {prospect.contactLinkedIn && (
              <div className="flex items-center gap-3">
                ...
                <a href={prospect.contactLinkedIn} target="_blank" rel="noopener noreferrer" className="text-sm text-violet-400 hover:text-violet-300">LinkedIn Profile</a>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Business Info */}
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
          <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-4">Business Details</h3>
          <div className="grid grid-cols-2 gap-4">
            {prospect.googleRating && (
              <div>
                <p className="text-[10px] text-slate-600 uppercase tracking-wider">Google Rating</p>
                <p className="text-sm text-amber-400 font-semibold mt-0.5">
                  {prospect.googleRating} <span className="text-slate-500 font-normal">({prospect.googleReviewCount} reviews)</span>
                </p>
              </div>
            )}
            <div>
              <p className="text-[10px] text-slate-600 uppercase tracking-wider">Campaign</p>
              <Link href={`/campaigns/${prospect.campaign.id}`} className="text-sm text-violet-400 hover:text-violet-300 mt-0.5 block">
                {prospect.campaign.name}
              </Link>
            </div>
            <div>
              <p className="text-[10px] text-slate-600 uppercase tracking-wider">Industry</p>
              <p className="text-sm text-slate-300 mt-0.5">{prospect.campaign.targetIndustry}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-600 uppercase tracking-wider">Location</p>
              <p className="text-sm text-slate-300 mt-0.5">{addressStr ?? prospect.campaign.targetCity}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-600 uppercase tracking-wider">First Contact</p>
              <p className="text-sm text-slate-400 mt-0.5">{new Date(prospect.createdAt).toLocaleDateString()}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-600 uppercase tracking-wider">Last Activity</p>
              <p className="text-sm text-slate-400 mt-0.5">{new Date(prospect.updatedAt).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {replyMessage && (
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
          <div className="flex items-center gap-3 mb-4">
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Their Reply</h3>
          ...
          <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-4">
            <p className="text-sm text-slate-300 leading-relaxed whitespace-pre-wrap">{replyMessage.replyBody}</p>
          </div>
        </div>
      )}
        ...
        {prospect.messages.length === 0 ? (
          ...
          <div className="space-y-4">
            {prospect.messages
              ...
              .map((msg) => (
              <div key={msg.id} className="border border-white/[0.04] rounded-lg overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 bg-white/[0.02]">
                  <div className="flex items-center gap-3">
                    {msg.replyBody ? (
                      ...
                      <span className="text-[10px] font-medium px-2 py-0.5 rounded bg-slate-500/15 text-slate-400 border border-slate-500/25">
                        Step {msg.stepNumber ?? '?'}
                      </span>
                    )}
                    <span className="text-sm text-slate-300">{msg.subject ?? '(no subject)'}</span>
                  </div>
                ...
                {msg.replyBody ? (
                  ...
                  <div className="px-4 py-3 border-t border-white/[0.04]">
                    <div className="text-sm text-slate-400 [&_p]:mb-2 [&_a]:text-violet-400" dangerouslySetInnerHTML={{ __html: msg.body }} />
                  </div>
                ) : null}
              </div>
            ))}
