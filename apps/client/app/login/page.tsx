...
interface Particle {
  ...
  radius: number; opacity: number; violet: boolean;
}
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < CONNECT_DIST) {
            ...
            ctx.strokeStyle = (a.violet && b.violet)
              ...
              : (!a.violet && !b.violet) ? `rgba(99,102,241,${alpha})` : `rgba(120,87,244,${alpha})`;
            ctx.lineWidth = 0.8;
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ctx.stroke();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0c0a18] relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md px-4">
        ...
        <p className="text-center text-[11px] text-slate-700 mt-6">Powered by Embedo &middot; AI infrastructure for your business</p>
      </div>
    </div>
  );
}
