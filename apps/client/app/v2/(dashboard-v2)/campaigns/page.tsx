'use client';
import { useBusiness } from '../../../../components/auth/business-provider';
import { PageShell, Card, Button, Spinner, EmptyState } from '../../components';

export default function V2Campaigns() {
  const { loading } = useBusiness();
  if (loading) return <Spinner />;

  return (
    <PageShell title="Campaigns" subtitle="Email & SMS marketing campaigns"
      actions={<Button variant="primary" size="sm">+ New Campaign</Button>}>
      <Card>
        <EmptyState
          icon={<svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5"><path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" /><path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" /></svg>}
          title="No campaigns yet"
          description="Create email and SMS campaigns to engage your customers."
          action={<Button variant="primary" size="sm">Create First Campaign</Button>}
        />
      </Card>
    </PageShell>
  );
}
