...
function ImagePicker({ businessId, selectedUrl, onSelect }: { businessId: string; selectedUrl: string | null; onSelect: (url: string | null) => void }) {
  ...
  if (loading) {
    return <div className="flex items-center gap-2 text-xs text-slate-400 py-2"><div className="w-3 h-3 border-2 border-violet-400 border-t-transparent rounded-full animate-spin" /> Loading images...</div>;
  }
      ...
      <div className="grid grid-cols-5 gap-1.5 max-h-36 overflow-y-auto pr-1">
        {images.map((img) => (
          <button key={img.id} type="button" onClick={() => onSelect(selectedUrl === img.url ? null : img.url)}
            className={`relative aspect-square rounded-lg overflow-hidden border-2 transition-all ${selectedUrl === img.url ? 'border-violet-500 ring-2 ring-violet-500/30' : 'border-slate-200 hover:border-slate-300'}`}>
  ...
  return (
    <ModalBackdrop onClose={onClose}>
      ...
      <div className="px-6 py-5 space-y-4 overflow-y-auto">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-500 mb-2">Image</label>
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          <span className="text-xs text-slate-600 font-medium">Publish immediately after generating</span>
        </label>
...
function EditModal({ businessId, post, onDone, onClose }: { businessId: string; post: ContentPost; onDone: () => void; onClose: () => void }) {
  ...
  return (
    <ModalBackdrop onClose={onClose}>
      <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-800">Edit Post</h3>
        <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 transition-colors">
          <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
        </button>
      </div>
      <div className="px-6 py-5 space-y-4 overflow-y-auto">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-400 mt-1">Comma-separated, without #</p>
        </div>
        <div>
          <label className="block text-xs font-medium text-slate-500 mb-2">Image</label>
          <ImagePicker businessId={businessId} selectedUrl={imageUrl} onSelect={setImageUrl} />
        ...
        <div>
          <label className="block text-xs font-medium text-slate-500 mb-2">Schedule</label>
          <div className="grid grid-cols-2 gap-2">
          ...
          <p className="text-[10px] text-slate-400 mt-1">Clear to save as draft</p>
        </div>
        {error && <p className="text-xs text-red-500">{error}</p>}
      </div>
      <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex gap-2 justify-end">
