  ...
  return (
    <div className="p-8 animate-fade-up">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Image Library</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Generate AI images, save URLs, and use them across your website, surveys, and campaigns.</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setShowSaveForm(!showSaveForm)}
            className="flex items-center gap-2 px-4 py-2 border border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-slate-300 text-sm font-medium rounded-xl hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors"
          >
      ...
      {/* Save URL form */}
      {showSaveForm && (
        <div className="bg-white dark:bg-white/[0.04] dark:backdrop-blur-sm border border-slate-200 dark:border-white/[0.08] rounded-2xl p-6 mb-6">
          <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-3">Save Image URL</h3>
          <div className="flex gap-3">
            ...
            />
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="px-3 py-2.5 border border-slate-200 dark:border-white/[0.08] dark:bg-white/[0.06] dark:text-white rounded-xl text-sm text-slate-600">
      ...
      {showGenerator && (
        <div className="bg-white dark:bg-white/[0.04] dark:backdrop-blur-sm border border-slate-200 dark:border-white/[0.08] rounded-2xl p-6 mb-6">
          <h3 className="text-sm font-bold text-slate-800 dark:text-white mb-3">Generate Image with DALL-E 3</h3>
          <textarea
            value={prompt}
          ...
          <div className="flex items-end gap-3">
            <div>
              <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Size</label>
              <div className="flex gap-1.5">
                {SIZES.map(s => (
                  <button key={s.value} onClick={() => setSize(s.value)} className={`px-3 py-1.5 text-xs font-medium rounded-lg border ${size === s.value ? 'border-violet-500 bg-violet-50 text-violet-700' : 'border-slate-200 dark:border-white/[0.08] text-slate-500 dark:text-slate-400'}`}>
                    {s.label.split(' ')[0]}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Quality</label>
              <div className="flex gap-1.5">
                {(['standard', 'hd'] as const).map(q => (
                  <button key={q} onClick={() => setQuality(q)} className={`px-3 py-1.5 text-xs font-medium rounded-lg border ${quality === q ? 'border-violet-500 bg-violet-50 text-violet-700' : 'border-slate-200 dark:border-white/[0.08] text-slate-500 dark:text-slate-400'}`}>
                    {q === 'hd' ? 'HD' : 'Standard'}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Category</label>
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="px-3 py-1.5 border border-slate-200 dark:border-white/[0.08] dark:bg-white/[0.06] dark:text-white rounded-lg text-xs text-slate-600">
                {CATEGORIES.filter(c => c !== 'all').map(c => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}
              </select>
            </div>
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            <span className="text-xs text-slate-600 dark:text-slate-300">Let AI rewrite my prompt for better results</span>
          </div>
      ...
      <div className="flex gap-2 mb-6">
        {CATEGORIES.map(c => (
          <button
            key={c}
            onClick={() => setFilter(c)}
            className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${filter === c ? 'border-violet-500 bg-violet-50 text-violet-700' : 'border-slate-200 dark:border-white/[0.08] text-slate-500 dark:text-slate-400 hover:border-slate-300'}`}
          >
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-white/[0.04] dark:backdrop-blur-sm border border-slate-200 dark:border-white/[0.08] rounded-2xl p-16 text-center">
          <div className="w-12 h-12 rounded-xl bg-violet-100 dark:bg-violet-500/20 flex items-center justify-center mx-auto mb-4">
          ...
          <h2 className="text-base font-semibold text-slate-800 dark:text-white mb-1">No images yet</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Generate AI images or save URLs to build your library.</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {images.map((img) => (
            <div
              key={img.id}
              className="group relative bg-white dark:bg-white/[0.04] dark:backdrop-blur-sm border border-slate-200 dark:border-white/[0.08] rounded-xl overflow-hidden cursor-pointer hover:shadow-lg hover:border-violet-200 transition-all"
              onClick={() => setSelectedImage(img)}
            >
              <div className="aspect-square overflow-hidden bg-slate-100 dark:bg-white/[0.06]">
              ...
              <div className="p-3">
                ...
                {img.category && <span className="inline-block mt-1 px-2 py-0.5 bg-slate-100 dark:bg-white/[0.06] text-slate-500 dark:text-slate-400 text-[10px] font-medium rounded-full">{img.category}</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-8" style={{ background: 'rgba(0,0,0,0.5)' }} onClick={() => setSelectedImage(null)}>
          <div onClick={(e) => e.stopPropagation()} className="relative bg-white dark:bg-[#1a1730] rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto my-auto">
            {/* Close button */}
            <button
            ...
            <div className="aspect-video overflow-hidden rounded-t-2xl bg-slate-100 dark:bg-white/[0.06]">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 text-xs text-slate-500 dark:text-slate-400">
                <span className="px-2 py-1 bg-slate-100 dark:bg-white/[0.06] rounded-full">{selectedImage.source === 'dalle' ? 'AI Generated' : selectedImage.source}</span>
                {selectedImage.category && <span className="px-2 py-1 bg-slate-100 dark:bg-white/[0.06] rounded-full">{selectedImage.category}</span>}
                <span className="px-2 py-1 bg-slate-100 dark:bg-white/[0.06] rounded-full">{new Date(selectedImage.createdAt).toLocaleDateString()}</span>
              </div>
              <div className="flex gap-2">
                <button onClick={() => copyUrl(selectedImage.url)} className="flex-1 py-2.5 border border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-slate-300 text-sm font-medium rounded-xl hover:bg-slate-50 dark:hover:bg-white/[0.04]">
