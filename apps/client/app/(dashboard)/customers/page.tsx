      ...
      <ModalBackdrop onClose={() => { onDone(); onClose(); }} wide>
        <div className="px-6 py-8 text-center">
          ...
          {success.mode === 'sequence' && steps.length > 1 && (
            <p className="text-xs text-slate-400 dark:text-slate-400 mb-4">{steps.length - 1} follow-up{steps.length > 2 ? 's' : ''} scheduled. Manage in any contact&apos;s Email Sequences section.</p>
          )}
    ...
    <div className="p-8 animate-fade-up">
        ...
        <div className="bg-white dark:bg-white/[0.04] dark:backdrop-blur-sm border border-slate-200 dark:border-white/[0.08] rounded-xl p-5 mb-8">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-400 mt-0.5">Email sequences running across your contacts</p>
            </div>
            <span className="text-xs text-slate-400 dark:text-slate-400">{sequences.filter((s) => s.active).length} active</span>
          </div>
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-[10px] text-slate-400 dark:text-slate-400">{new Date(seq.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                      </div>
                    </div>
                  </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <span className="text-xs text-slate-400 dark:text-slate-400 ml-auto">{total} result{total !== 1 ? 's' : ''}</span>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 dark:border-white/[0.06]">
                ...
                <th className="text-left text-[10px] font-semibold text-slate-400 dark:text-slate-400 uppercase tracking-wider px-5 py-3">Email</th>
                <th className="text-left text-[10px] font-semibold text-slate-400 dark:text-slate-400 uppercase tracking-wider px-5 py-3">Phone</th>
                <th className="text-left text-[10px] font-semibold text-slate-400 dark:text-slate-400 uppercase tracking-wider px-5 py-3">Source</th>
                ...
                <th className="text-left text-[10px] font-semibold text-slate-400 dark:text-slate-400 uppercase tracking-wider px-5 py-3">Added</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                contacts.map((contact) => (
                  <tr key={contact.id} className={`border-b border-slate-50 dark:border-white/[0.04] hover:bg-slate-50/50 dark:hover:bg-white/[0.04] transition-colors ${selectedIds.has(contact.id) ? 'bg-violet-50/30 dark:bg-violet-500/10' : ''}`}>
                    ...
                    <td className="px-5 py-3 text-sm text-slate-400 dark:text-slate-400 cursor-pointer" onClick={() => router.push(`/customers/${contact.id}`)}>{formatDate(contact.createdAt)}</td>
                  </tr>
                ))
              )}
            </tbody>
