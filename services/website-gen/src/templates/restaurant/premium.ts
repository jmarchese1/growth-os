export type SectionKey = 'about' | 'features' | 'menu' | 'gallery' | 'testimonials' | 'hours' | 'reserve';
...
const COLOR_SCHEMES: Record<ColorScheme, { bg: string; surface: string; text: string; muted: string; accent: string; accentText: string; border: string }> = {
  ...
  slate:    { bg: '#0f172a', surface: '#1e293b', text: '#f1f5f9', muted: '#64748b', accent: '#94a3b8', accentText: '#0f172a', border: '#334155' },
  emerald:  { bg: '#022c22', surface: '#064e3b', text: '#ecfdf5', muted: '#6ee7b7', accent: '#10b981', accentText: '#fff', border: '#065f46' },
  amber:    { bg: '#1c0a00', surface: '#2d1600', text: '#fffbeb', muted: '#b45309', accent: '#f59e0b', accentText: '#fff', border: '#451a03' },
  crimson:  { bg: '#0c0404', surface: '#1c0a0a', text: '#fff5f5', muted: '#9f6060', accent: '#dc2626', accentText: '#fff', border: '#2d0a0a' },
  navy:     { bg: '#030712', surface: '#0f172a', text: '#f8fafc', muted: '#6366f1', accent: '#818cf8', accentText: '#030712', border: '#1e1b4b' },
  sage:     { bg: '#f4f5f0', surface: '#ffffff', text: '#2d3320', muted: '#8a9070', accent: '#6b7c5e', accentText: '#fff', border: '#d4d9c8' },
};
...
const FONT_PAIRINGS: Record<FontPairing, { heading: string; body: string; googleFont?: string }> = {
  ...
  luxury:    { heading: "'Cormorant Garamond', serif",       body: "'Cormorant', serif",              googleFont: 'Cormorant+Garamond:ital,wght@0,300;0,400;0,600;1,400&family=Cormorant:wght@300;400' },
...
function hoursTable(hours?: Record<string, string>, c?: Colors): string {
  ...
  const cards = config.testimonials.map((t) => `
    <div style="background:${c.surface};border:1px solid ${c.border};border-radius:20px;padding:36px;display:flex;flex-direction:column;gap:20px;">
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-top:auto;">
        <div style="width:40px;height:40px;border-radius:50%;background:${c.accent}25;display:flex;align-items:center;justify-content:center;font-family:${f.heading};font-size:16px;font-weight:700;color:${c.accent};">${t.author.charAt(0)}</div>
        <div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;">
        ...
        <div style="background:${c.bg};border:1px solid ${c.border};border-radius:20px;padding:36px;display:flex;flex-direction:column;gap:24px;">
          <div>
            ...
            <p style="font-size:17px;color:${c.text};font-weight:500;line-height:1.5;">${config.address ?? (config.city ?? 'Contact us for address')}</p>
          </div>
...
// ── Nav link helpers ─────────────────────────────────────────────────────────
function navLinks(activeSections: SectionKey[], config: PremiumWebsiteConfig, _c: Colors): string {
  const links: string[] = [];
  if (activeSections.includes('about'))   links.push(`<a href="#about" class="nav-link">About</a>`);
