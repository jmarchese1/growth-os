...
logger.info('website-gen routes loaded — version: 2026-03-19-v7-tailwind-unsplash');
...
Editable: businessName, tagline, description, cuisine, phone, address, city, hours, heroImage, bookingUrl, colorScheme, fontPairing, heroHeading, heroSubheading, aboutHeading, aboutBody, ctaText, menuItems`,
  ...
  app.get<{ Querystring: { query: string } }>('/search-pexels', async (req, reply) => {
    ...
    try {
      // Fetch multiple pages to get ~200 results
      ...
      for (let page = 1; page <= 3; page++) {
        ...
        if (res.ok) {
          ...
          for (const p of data.photos) {
            allPhotos.push({ url: p.src.large, alt: p.alt || query, photographer: p.photographer });
          }
        }
        if (allPhotos.length >= 200) break;
      }
      if (allPhotos.length > 0) {
        return reply.send({ success: true, images: allPhotos.slice(0, 200) });
      }
    } catch { /* silent */ }
